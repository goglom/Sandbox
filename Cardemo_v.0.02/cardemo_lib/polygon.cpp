...
polygon::polygon(const vector_2d<float>& center, 
	const std::initializer_list<vector_2d<float>> list, float angle)
...
{
	if (abs(angle) < numeric_limits<float>::epsilon())
	{
		return;	
	}
...
void polygon::move_n_rotate(const vector_2d<float>& position, float angle)
{
	center_ = position;
...
vector_2d<float> polygon::get_nearest_normal(const vector_2d<float>& point) const
{
	auto min_dist_to_this = (*wc_vertices_.begin() - point).length();
	auto min = wc_vertices_.begin();
	
	for (auto it = wc_vertices_.begin(); it != wc_vertices_.end(); ++it)
	{
		if ((*it - point).length() < min_dist_to_this)
		{
			min_dist_to_this = (*it - point).length();
			min = it;
		}
	}

	const auto prev_vertex = min == wc_vertices_.begin()
		                         ? wc_vertices_.back()
		                         : *(min - 1);

	const auto next_vertex = (min + 1) == wc_vertices_.end()
		                         ? wc_vertices_.front()
		                         : *(min + 1);

	const auto first_scal_mul = (*min - point) * (prev_vertex - point);
	const auto second_scal_mul = (*min - point) * (next_vertex - point);

	if (first_scal_mul < 0)
	{
		return (*min - prev_vertex).get_orthogonal();
	}
	if (second_scal_mul < 0)
	{
		return (next_vertex - *min).get_orthogonal();
	}
	if (first_scal_mul >= 0  && second_scal_mul >= 0)
	{
		return (*min - prev_vertex).get_orthogonal()
			+ (next_vertex - *min).get_orthogonal();
	}
	return {};
}

vector_2d<float> polygon::find_nearest_point(const vector_2d<float>& point) const
{
	auto min_dist_to_this = (*wc_vertices_.begin() - point).length();
	auto min = wc_vertices_.begin();
...
vector_2d<float> polygon::get_wc_vertex(size_t index) const
{
	return wc_vertices_[index];
}
